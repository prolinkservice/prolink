...
| 兩段式提醒只留出發前那次 | 省一半（第三批做） |

一筆預約走完全程約 4 則 → 免費 200 則約等於 **50 筆**。**一個月穩定超過的職人，正解是升級 LINE 官方帳號的輕用量方案（約 NT$800／月含 3,000 則）**，而不是省到不敢發通知。用量看板要直接把這句話寫出來。
...
| **2** | 自帶 LINE 官方帳號串接 + 一對一預約流程 + 兩段式提醒 + 訊息用量看板 | 2 週 | 🔨 第二批完成 |
...
2. Sprint 2 第三批：兩段式行前提醒（前一晚 + 出發前 2 小時）
