...
#### 24 小時內的預約跳過確認（2026-08-02 定案）

「24 小時未確認自動釋出」在**開始時間本來就在 24 小時內**的預約上會失效——釋出時間點會晚於預約本身，那筆會永遠卡在 `pending`，變成一筆沒人管的單。

| 開始時間 | 送出後狀態 | 客人收到 |
|---|---|---|
| 24 小時以後 | `pending` | 「請確認你的預約」卡片，有確認／取消兩顆按鈕 |
| **24 小時以內** | **`confirmed`（直接成立）** | 「預約成立」卡片，有**「我無法前往」**按鈕 |

理由：確認機制的目的是防止客人忘記，而剛剛才自己送出、明天就要來的預約本來就不會忘。這樣釋出時間永遠早於預約時間，矛盾從根本消失，也不必跟職人解釋兩套計時規則。反悔的路仍然留著。

#### 沒綁 LINE 的客人不能留在待確認

付費方案的客人若沒有綁 LINE（自己存網址、從官網進來），他收不到那則要確認的訊息，卻會被排程釋出。因此**送出後若確認訊息發不出去，該筆直接改為 `confirmed`**，並回傳改過的狀態給成功頁，不讓客人等一則永遠不會來的訊息。

#### 釋出排程一天一次，靠第二個條件補洞

**Vercel 的 Hobby 方案限制排程一天只能觸發一次**，改成每小時會讓整個部署失敗（錯誤指向 cron 計費頁）。維持 `0 3 * * *`。

單靠「送出滿 24 小時就釋出」配上一天一次會留洞：

```
週一 04:00 送出，約週二 05:00（25 小時後 → 要確認）
週二 03:00 掃描 → 才過 23 小時，不釋出
週二 05:00 時間到了，那筆還卡在待確認，時段整段鎖著沒人能約
```
...
| 條件 | 用途 |
|---|---|
| `created_at` 超過 24 小時 | 正常情況 |
| **`start_at` 進入 12 小時內** | 補上面那個洞。因為 24 小時內送出的預約直接成立、不會有 pending，待確認的預約一定至少隔著一次掃描 |

升級 Vercel 方案後改回 `0 * * * *` 就會更即時。
...
| 行前提醒 | **每天中午 12:00 提醒明天要來的客人**（2026-08-03 定案，見 §6.5.1） |
| **提醒訊息內含「無法前往」按鈕** | 取消愈好按，放鳥愈少。夯客的提醒沒有按鈕，這是差異點 |
| 新客須加 LINE 好友才能預約 | 可開關 |
| 客戶卡信賴標記 | 顯示「到店 12 · 放鳥 1 · 臨時取消 2」 |

#### 6.5.1 行前提醒改成一天一次（2026-08-03 定案）

草稿：[`mockups/booking-reminders.html`](mockups/booking-reminders.html)

原訂「前一晚 20:00 ＋ 出發前 2 小時」兩段。**出發前那段需要每小時掃一次，而 Vercel Hobby 的排程一天只觸發一次、整個專案只能有兩個**，所以先做成一天一次。

| 規則 | 決定 |
|---|---|
| 什麼時候發 | 每天 **12:00（租戶時區）**，與夯客同一個節奏。UTC 寫 `0 4 * * *` |
| 發給誰 | 掃「**現在起 36 小時內開始**、已確認、還沒提醒過」的預約，每筆一則 |
| 為什麼是 36 小時而不是「明天整天」 | 正常情況兩者結果相同，但 36 小時多做到兩件事：**昨天中午之後才臨時成立的補得到**、**排程漏跑一天不會整批消失** |
| 實際提前量 | 開始前 12–36 小時，與夯客相同 |
| 訊息內容 | 「明天 14:00 有預約」＋項目／時間／地點／職人自訂的一句叮嚀＋**看地圖**＋**我無法前往** |
| 「今天／明天」 | 用租戶時區換算。寫「8/4（一）14:00」客人還要自己對日曆 |
| 只發一次 | `bookings.reminder_sent_at`。發失敗也蓋章——下次掃描是 24 小時後，那時多半來不及 |
| 職人關掉時 | **不蓋章**，他明天打開就還來得及 |
| 排程位子 | 從 `send-followup-messages` 換過來。那支讀的是 `drop_legacy` 已砍掉的表，本來就每天失敗一次 |

> 之後若改成每小時跑（GitHub Actions 免費，或 Vercel Pro），把「出發前 2 小時」那段加回來不用改這支程式——它本來就跟排程頻率無關。
...
### 9.1.1 客人怎麼綁上 LINE（2026-08-02 定案）

LINE 的 follow 事件只給一串 `line_user_id`，**沒有姓名也沒有電話**，所以系統不知道加好友的是哪一位客人。要能發通知，必須有一次把「代號」與「手機號碼」接起來的動作。

**採 A 案：歡迎訊息裡的預約按鈕帶加密記號。**

```
客人加好友
  → 系統用職人的名義回覆歡迎訊息（reply，不計額度）
  → 訊息裡的「立即預約」按鈕網址帶 ?ref={AES-256-GCM 加密的 tenantId + lineUserId + 到期時間}
  → 客人照常選服務、選時段、填手機送出
  → create_booking 收到 p_line_user_id → 寫進 customers.line_user_id
  → 綁定完成，之後所有通知發得出去
```
...
|---|---|
| 記號有效期 | 30 天 |
| 解不開時 | 一律當作沒有綁定資訊，照常建立預約。綁不到只是收不到通知，不該讓客人約不成 |
| 換手機號碼再約 | 同一組代號只能掛一位客人（唯一約束）。建單前先從舊那筆放開，否則整筆預約會失敗 |
| **關鍵字補綁** | 客人在對話裡打「預約 / 我要預約 / 訂位 / 約時間」，回同一張帶記號的卡片。**早就加了好友、歡迎訊息已滑不見的舊客人靠這條補綁**，且走 reply 不計額度 |
| 對不上的訊息 | **一律不回。** 罐頭訊息會蓋掉職人自己想講的話，也讓客人以為在跟機器人講話 |
| 職人自己打關鍵字 | 不觸發。那個位置留給 §9.4「一句話建立預約」 |
| **與 LINE 後台的衝突** | LINE Official Account Manager 的「加入好友的歡迎訊息」與「自動回應訊息」**必須關掉**，否則客人會收到兩則。後台的 LINE 設定頁直接列出這四個開關（`ConsoleChecklist`），不能指望職人自己知道 |
| 綁不到的人 | 自己存網址、從官網／IG 進來的客人。客戶名單顯示「LINE 未綁」，補綁工具排 Sprint 4 |
| 被封鎖 | 發送失敗或收到 unfollow 事件時寫 `customers.line_blocked_at`，名單顯示「已封鎖我」 |

> 不採「請客人回覆手機號碼」：摩擦太高，會把剛加的好友嚇跑。
> 不採 LIFF：職人要多申請一組設定，而且 LIFF 一樣只給代號，還是要對手機——多一道工，沒解決問題。
...
**不繞過 LINE 的計費，那是違反條款、會被停權的做法**——職人的官方帳號被停等於他的客人全部聯絡不到。合法的做法有四層：

| 做法 | 效果 |
|---|---|
| **能用回覆就不用推播** | 客人傳訊息或按按鈕後 30 秒內的 reply **不計額度**。歡迎訊息、確認後的答覆全部走這條 |
| 資訊塞進**同一張 Flex 卡片** | LINE 按訊息則數計費，「文字＋卡片」就是兩則 |
| 關掉「有新預約時也通知我」 | 約省四分之一。設定在 `tenant_settings.notify_self_on_new_booking`，預設開 |
| 行前提醒改成一天一則 | 原訂兩段，實作成一天一次（§6.5.1），本來要花的 2 則變 1 則 |

一筆預約走完全程約 5 則（含行前提醒）→ 免費 200 則約等於 **40 筆**。**一個月穩定超過的職人，正解是升級 LINE 官方帳號的輕用量方案（約 NT$800／月含 3,000 則）**，而不是省到不敢發通知。用量看板要直接把這句話寫出來。
...
2. ~~先選時間，再選地點~~ → **改為先選地點**（2026-08-02，見 §9.5.1）
3. **提醒訊息一定要有「取消」按鈕**

#### 9.5.1 多據點時先選地點（2026-08-02 定案）

草稿：[`docs/mockups/booking-location-first.html`](mockups/booking-location-first.html)

原本「先選時間再選地點」是為**純到府／流動型**職人設計的。實際導入的第一家（揚翼）是**兩間固定店面**，兩排時段疊在同一頁要一直往下捲，而地址擠在分組標題後面很難讀——**客人不知道地址根本決定不了要去哪**。

流程改為 **選服務 → 選地點 → 選時段**：

| 規則 | 決定 |
|---|---|
| 什麼時候多這一步 | 只有 `location_mode = multi_site` 且據點 ≥ 2。**`fixed` 不問**——那項服務在別間根本沒開，讓客人選到只會看到空白 |
| 單一據點 | 自動跳過，流程完全不變 |
| 到府（`mobile`） | 跳過。地址是客人自己家，最後填資料時才問 |
| 地點卡片顯示 | 名稱、**地址獨立一行**、「今天還有 N 個時段」（0 個顯示「今天沒有空檔」）、看地圖連結 |
| 選定之後 | 標題換成地點名、時段不再分組、「‹」回到換地點 |
| 步驟數 | 有選地點時 1／3，否則維持 1／2 |

> **代價**：客人看不到「另一間今天有更早的時段」。用卡片上的「今天還有 N 個時段」補救，讓他在點進去之前就比得出來。
>
> **選地點後要重問一次時段，不能拿手邊的資料自己篩**：「最近可以約的是哪一天」必須只看那一間，否則客人會在五甲的頁面上看到六合才有的日期。
...
| **2** | 自帶 LINE 官方帳號串接 + 一對一預約流程 + 行前提醒 + 訊息用量看板 | 2 週 | 🔨 第三批完成 |
...
### 14.1 目前進度（2026-08-03 · 第三批）

**行前提醒寫完，尚未實測**

| 功能 | 狀態 |
|---|---|
| 每天 12:00 提醒明天要來的客人（§6.5.1） | build 綠燈、`npm run preflight` 綠燈，**未實測** |
| 提醒卡片：看地圖 ＋ 我無法前往（接現有取消流程） | build 綠燈，**未實測** |
| 後台開關 ＋ 「加一句自己的話」（`reminder_note`） | build 綠燈 |
| 用量看板列出「行前提醒」這一類 | build 綠燈 |

**⚠️ 這批一樣有一支沒套用的 migration**：`20260802000005_booking_reminders.sql`。
沒套用之前，LINE 設定頁會因為讀不到 `reminder_enabled` 出錯，排程也會因為找不到 `reminder_sent_at` 整批失敗。

**排程動過**：`vercel.json` 把 `send-followup-messages`（舊系統殘骸，讀的表在 `drop_legacy` 就砍了，每天失敗一次）換成 `send-reminders`，時間 `0 4 * * *`（UTC）＝ 台北 12:00。Hobby 方案只給兩個排程位子。

實測方式：把某筆已確認預約的 `start_at` 改到 20 小時後、`reminder_sent_at` 設回 null，然後手動打
`curl -H "Authorization: Bearer $CRON_SECRET" https://prolink-delta.vercel.app/api/cron/send-reminders`。

---

### 14.2 先前進度（2026-08-02 · 第二批）

**Sprint 2 第二批寫完，但尚未實測**

| 功能 | 狀態 |
|---|---|
| 客人加好友 → 歡迎訊息（含綁定記號的預約按鈕） | build 綠燈，**未實測** |
| 客人打「預約」等關鍵字 → 回同一張卡片（舊客補綁） | build 綠燈，**未實測** |
| 客人從那顆按鈕預約 → 自動綁定 LINE | build 綠燈，**未實測** |
| 預約成立 → 請客人確認 / 直接成立（24 小時內） | build 綠燈，**未實測** |
| 客人按「確認預約」「我要取消」「我無法前往」 | build 綠燈，**未實測** |
| 職人在後台取消未發生的預約 → 通知客人 | build 綠燈，**未實測** |
| 客戶名單顯示 LINE 已綁／未綁／已封鎖 | build 綠燈 |
| 用量看板拆類別 + 新預約通知開關 + 歡迎詞自訂 | build 綠燈 |
| 未確認自動釋出（改每小時跑）+ 釋出通知 | build 綠燈，**未實測** |

**⚠️ 上線前一定要做的一件事**

`supabase/migrations/20260802000001_line_notifications.sql` **還沒套用到正式資料庫**。
沒套用之前，客戶列表、今日行程、行事曆、LINE 設定頁都會因為讀不到新欄位而出錯。

實測順序建議：① 用測試帳號加好友看有沒有歡迎訊息 → ② 從那顆按鈕預約 → ③ 檢查客戶名單是不是變「LINE 已綁」→ ④ 按確認 → ⑤ 後台取消一筆看客人收不收得到。

---

### 14.3 先前進度（2026-08-02 · 第一批）
...
1. ~~Sprint 2 第二批~~ ✅ 寫完，待實測
2. ~~Sprint 2 第三批：行前提醒~~ ✅ 寫完，待實測（§6.5.1）
3. Sprint 2 收尾：圖文選單一鍵套用、老師在 LINE 一句話建立預約
4. 客人詳情頁（預約歷史、備註、手動封鎖）、CSV 匯入、**幫舊客補綁 LINE**
...
| [`mockups/line-notifications.html`](mockups/line-notifications.html) | 客人的 LINE 綁定與四種自動通知（Sprint 2 第二批） |
| [`mockups/booking-reminders.html`](mockups/booking-reminders.html) | 行前提醒：每天中午提醒明天的客人（Sprint 2 第三批） |
