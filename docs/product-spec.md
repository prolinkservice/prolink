...
| Sprint | 內容 | 估時 |
|---|---|---|
| **0** | Schema + RLS + 租戶骨架 + 認證改造（客人免註冊） | 1 週 |
| **1** | 按摩推拿完整可用：服務項目、營業時間、據點排班、線上預約頁、**手動建立預約**、行事曆、結帳登記 | 2 週 |
| **2** | 自帶 LINE 官方帳號串接 + 一對一預約流程 + 兩段式提醒 + 訊息用量看板 | 2 週 |
| **3** | 場地租借：按小時、定期預約、場地表、**月底自動對帳單** | 1.5 週 |
| **4** | 客戶管理 CRM + 放鳥計點 + 黑名單 + CSV 匯入 | 1.5 週 |
| **5** | 定金 BYO 金流（銀行轉帳 + LINE Pay + INSTO）+ 退款待辦 + 對帳 | 1.5 週 |
| **6** | 地點對矩陣 + 服務區域 + 今日路線檢視 | 1 週 |
| **7** | 訂閱計費 + 報表 | 1.5 週 |
| **8+** | 群組機器人、團課（成團／流團／候補）、次卡儲值 | — |
